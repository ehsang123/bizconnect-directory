...
const useSeo = () => {
  useEffect(() => {
    ...
    const description =
      "Explore managed security service providers (MSSPs) delivering SOC, monitoring and MDR.";
...
const ManagedSecurityServices = () => {
  ...
  const [initialCompanies] = useState<Company[] | undefined>(() => {
    ...
    try {
      const stored = window.localStorage.getItem("companies_cache_mssp_v1");
  ...
  const { data: companies, isLoading } = useQuery({
    queryKey: ["companies", "approved", "mssp"],
    queryFn: async () => {
      ...
      if (typeof window !== "undefined") {
        try {
          window.localStorage.setItem("companies_cache_mssp_v1", JSON.stringify(typedData));
        } catch {
          // ignore cache errors
        }
      }
      ...
      <section className="container py-8 md:py-10">
        <div className="mb-6 space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Browse approved MSSP partners. Use filters for country, industry and company size to find
        ...
        <div className="grid gap-8 lg:grid-cols-[280px,1fr]">
          ...
          <div className="space-y-4">
            <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
              <div>
                <h2 className="text-xl font-semibold tracking-tight">All MSSPs</h2>
                <p className="text-sm text-muted-foreground">
                  {isLoading
